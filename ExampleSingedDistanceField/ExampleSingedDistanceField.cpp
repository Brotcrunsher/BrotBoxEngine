...
public:
	...
	virtual void draw2D(bbe::PrimitiveBrush2D & brush) override
	{
		...
		camPos.z *= -1;
		sdfShader.setPushConstant(brush, 128, sizeof(float), &camPos.x);
		sdfShader.setPushConstant(brush, 132, sizeof(float), &camPos.z);
		sdfShader.setPushConstant(brush, 136, sizeof(float), &camPos.y);

		bbe::Vector3 forward = ccnc.getCameraForward();
		forward.z *= -1;
		sdfShader.setPushConstant(brush, 140, sizeof(float), &forward.x);
		sdfShader.setPushConstant(brush, 144, sizeof(float), &forward.z);
		sdfShader.setPushConstant(brush, 148, sizeof(float), &forward.y);
