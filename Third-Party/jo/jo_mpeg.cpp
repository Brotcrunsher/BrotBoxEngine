...
void bufferPanic()
{
	std::cout << "jo_mpeg: Buffer Panic!" << std::endl;
	exit(100);
}

void putc_(char val, RamFile& ptr)
{
	if (ptr.amount + 1 >= ptr.bufferSize)
	{
		bufferPanic();
	}
	*ptr.writeHead = val;
...
void fwrite_(void const* src, size_t elemSize, size_t elemCount, RamFile &ptr)
{
	size_t amount = elemSize * elemCount;
	if (ptr.amount + amount >= ptr.bufferSize)
	{
		bufferPanic();
	}
	memcpy(ptr.writeHead, src, amount);
	ptr.amount += amount;
	ptr.writeHead += amount;
}
...
void jo_write_mpeg(char* buffer, size_t bufferSize, size_t &out_amount, const unsigned char *rgbx, int width, int height, int fps, bool requiresSwizzle) {
	RamFile fp;
	fp.amount = 0;
	fp.writeHead = buffer;
	fp.bufferSize = bufferSize;
	
	int lastDCY = 128, lastDCCR = 128, lastDCCB = 128;
	jo_bits_t bits = {&fp};

	// Sequence Header
	fwrite_("\x00\x00\x01\xB3", 4, 1, fp);
	// 12 bits for width, height
	putc_((width>>4)&0xFF, fp);
	putc_(((width&0xF)<<4) | ((height>>8) & 0xF), fp);
	putc_(height & 0xFF, fp); 
	// aspect ratio, framerate
	if(fps <= 24) putc_(0x12, fp);
	else if(fps <= 25) putc_(0x13, fp);
	else if(fps <= 30) putc_(0x15, fp);
	else if(fps <= 50) putc_(0x16, fp);
	else putc_(0x18, fp); // 60fps
	fwrite_("\xFF\xFF\xE0\xA0", 4, 1, fp);

	fwrite_("\x00\x00\x01\xB8\x80\x08\x00\x40", 8, 1, fp); // GOP header
	fwrite_("\x00\x00\x01\x00\x00\x0C\x00\x00", 8, 1, fp); // PIC header
	fwrite_("\x00\x00\x01\x01", 4, 1, fp); // Slice header
	...
	fwrite_("\x00\x00\x01\xb7", 4, 1, fp); // End of Sequence

	out_amount = fp.amount;
}
#endif
